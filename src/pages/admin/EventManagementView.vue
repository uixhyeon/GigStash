    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm p-6">
          <div class="mb-6">
            <div class="flex justify-between items-center">
              <button @click="prevMonth" class="w-10 h-10 flex items-center justify-center bg-gray-100 dark:bg-slate-700 hover:bg-gray-200 dark:hover:bg-slate-600 rounded-lg transition-all flex-shrink-0 text-gray-600 dark:text-gray-400" title="이전 달">
                <i class="fi fi-br-angle-left text-2xl"></i>
              </button>
              <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ currentMonth }}</h3>
              <button @click="nextMonth" class="w-10 h-10 flex items-center justify-center bg-gray-100 dark:bg-slate-700 hover:bg-gray-200 dark:hover:bg-slate-600 rounded-lg transition-all flex-shrink-0 text-gray-600 dark:text-gray-400" title="다음 달">
          ...
          <div class="grid grid-cols-7 gap-2 mb-2">
            <div
              ...
              :class="[
                ...
                index === 0 ? 'day-sunday' : index === 6 ? 'day-saturday' : 'text-gray-600 dark:text-gray-400'
              ]"
            >
              {{ day }}
            </div>
          ...
          <div class="grid grid-cols-7 gap-2">
            <button
              ...
              :class="[
                ...
                date.isCurrentMonth
                  ? (index % 7 === 0 ? 'day-sunday-text' : index % 7 === 6 ? 'day-saturday-text' : 'text-gray-900 dark:text-white') + ' hover:bg-blue-50 dark:hover:bg-slate-700 cursor-pointer'
                  : 'text-gray-400 dark:text-gray-600',
                date.isToday ? 'ring-2 ring-blue-500 dark:ring-blue-400 font-semibold' : '',
                selectedDate && date.dateStr && new Date(date.dateStr).toDateString() === selectedDate.toDateString()
                  ? 'bg-blue-600 text-white'
                  : date.eventCount > 0 ? 'bg-blue-50 dark:bg-blue-900/20 font-semibold' : '',
              ]"
            >
              <span>{{ date.date }}</span>
              <!-- 행사별 색상 표시 -->
              <div v-if="date.eventCount > 0 && date.isCurrentMonth" class="flex gap-1 mt-1 flex-wrap justify-center">
          ...
          <div class="mt-6 pt-6 border-t border-gray-200 dark:border-slate-700">
            <h4 class="text-sm font-semibold mb-3 text-gray-700 dark:text-gray-300">
              {{ selectedDate ? `${selectedDate.getMonth() + 1}월 ${selectedDate.getDate()}일 행사` : '선택된 행사' }}
            </h4>
            <div v-if="selectedDateEvents.length === 0" class="text-xs text-gray-500 dark:text-gray-400 py-2">
              행사가 없습니다.
            </div>
            <div class="space-y-2">
              <div v-for="event in selectedDateEvents" :key="event.id" class="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg text-sm">
                <div class="font-medium text-gray-900 dark:text-white">{{ event.name }}</div>
                <div class="text-xs text-gray-600 dark:text-gray-400 mt-1">{{ event.startDate }} ~ {{ event.endDate }}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm overflow-hidden">
        <table class="w-full text-sm">
          ...
          <tbody>
            <tr v-if="filteredEvents.length === 0" class="border-t border-slate-200 dark:border-slate-700">
              <td colspan="6" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">
                검색 결과가 없습니다.
              </td>
            </tr>
            <tr v-for="event in filteredEvents" :key="event.id" class="border-t border-slate-200 dark:border-slate-700">
              ...
              <td class="px-4 py-3">
                ...
                >
                  {{ event.status === '진행 중' ? event.status : event.status === '예정' ? event.status : '종료' }}
                </span>
              </td>
              <td class="px-4 py-3 text-slate-900 dark:text-slate-100">{{ event.participants }}명</td>
